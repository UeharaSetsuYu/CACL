...
def main_train(args, dataset_name, config, device):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in loop:
            ...
            if flag:
                visualize_raw_images_grid(data_list[0], data_list[1], target_list[0], 5)
                flag = False
            ...
            if epoch < pre_train:
                ...
                loss_rec += reconstruction(model, data_list, view_num, Criterion, optimizer_Rec, target_list, Flag, mask)
            ...
            loss_metrics['Consistency_Loss'].append(loss_consis)
            loss_metrics['Discriminator'].append(loss_adver_disc)
            loss_metrics['Classifier_Loss'].append(loss_adver_cla)
...
def data_preprocess(dataset_name, args) ->list:
    ...
    for i in range(view_num):
        ...
        data_train_loader = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=False, drop_last=True)    # 已经打乱数据了不需要再次打乱(shuffle = False)
